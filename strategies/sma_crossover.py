...
Maintains fast and slow simple moving averages over ticker last-prices. A
fast-over-slow crossover emits a market buy; a fast-under-slow crossunder
emits a market sell that closes the position. Intended as a minimal,
easily-testable reference strategy.
"""
...
class SmaCrossoverStrategy(BaseStrategy):
    """Golden-cross / death-cross strategy on streaming tickers.

    Args:
        symbol: Market to trade, e.g. ``"BTC/EUR"``.
        fast_period: Window of the fast SMA (ticks).
        slow_period: Window of the slow SMA (ticks); must exceed ``fast_period``.
    ...
    async def on_ticker(self, ticker: Ticker) -> None:
        """Update the SMAs and detect crossovers."""
        self._last_price = ticker.last
        self._fast_prices.append(ticker.last)
        self._slow_prices.append(ticker.last)
        ...
        if self._prev_diff is not None:
            if self._prev_diff <= 0 < diff and self._position_amount == 0:
                amount = self._order_quote_size / ticker.last
